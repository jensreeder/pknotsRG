...
This variant of pknotsRG computes the best length normalized pseudoknot.
...
>	      (e,(p,s))  =  case (pknots input pkmaxsize (energy *** pp *** enum)) of
>				      []     -> (0,("The input sequence contains no pseudoknot!",
>				                 SS (0,inplength)))
>				      (x:xs) -> x 
>             ereal       :: Float
>             ereal       = (fromIntegral e / 100)			      
>	      (start,end) = compbounds s             in
>	      putStr (show start ++ replicate (end-start-3) '.' ++ show end ++ "\n"
>				 ++ drop start (take end input) ++"\n"
>			         ++ p ++ " (" ++ show ereal ++")\n")


> pknots ::  Ord a=>[Char] -> Int ->( (RNAInput -> FS_Algebra  Int a b)
> 		       ,(RNAInput -> FS_AlgebraExt  Int a b)) -> [a]

> pknots sequence maxpk algebra = axiom (unscale <<< bestPK) where
>   h_top []		= []
>   h_top xs		= [minimum xs]
>   tabulated		= table  n 
>   listed		= table1 n
...
>   (pk, pkmldl, pkmldr, pkmldlr, pkml, pk', kndl, kndr,  kndlr, frd, bkd, scale, unscale,
>	    emptymid, midbase, middlro, middl, middr, middlr, midregion, pss)    = algext inp
   
>   bestPK  = skipleft  <<< base    +~~ bestPK |||
>	                    bestPK1            ... h_top
>   bestPK1 = skipright <<< bestPK1 ~~+ base   |||
>			    knotrel            ... h_top            
>   knotrel = scale     <<< loc .~~ knot ~~. loc           
>
...
>		     sumend  lb _ rb   = 0 
    ...
    We need this as a correction term in case of overlapping pseudoknotstems
...
> compbounds :: Component a -> (Int, Int)
> compbounds (PK (i,j) _ _ _ _ _ (k,l)) = (i,l)
> compbounds (SS (i,j))                 = (i,j)
> compbounds _ = error ("function not defined for this Component")
